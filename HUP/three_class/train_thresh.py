...
for t1 in threshold_range:
    for t2 in threshold_range:
        preds = []
        for prob in val_logits:
            if prob[1] >= t1 and prob[1] >= prob[0] and prob[1] >= prob[2]:
                preds.append(1)
            elif prob[2] >= t2 and prob[2] >= prob[0] and prob[2] >= prob[1]:
                preds.append(2)
            else:
                preds.append(0)
        ...
        if f1 > best_score:
            best_score = f1
            best_thresh = {1: t1, 2: t2}
            best_report = classification_report(y_true, preds, digits=4)
            best_preds = preds

# Print and save best thresholds
print("Best Thresholds:")
print(f"  Class 1: {best_thresh[1]:.2f}")
print(f"  Class 2: {best_thresh[2]:.2f}")
print(f"  Macro F1: {best_score:.4f}")
print("\nClassification Report with Best Thresholds:\n")
print(best_report)

# Save for use in test set
...
final_pred = []
for prob in probs:
    if prob[1] >= thresholds[1] and prob[1] >= prob[0] and prob[1] >= prob[2]:
        final_pred.append(1)
    elif prob[2] >= thresholds[2] and prob[2] >= prob[0] and prob[2] >= prob[1]:
        final_pred.append(2)
    else:
        final_pred.append(0)
