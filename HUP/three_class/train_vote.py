...
val_preds = [np.argmax(p, axis=1) for p in merged_preds]
val_preds = np.stack(val_preds, axis=0)
val_pred, _ = mode(val_preds, axis=0)
...
hard_preds = [np.argmax(p, axis=1) for p in test_preds]  # list of arrays
...
# ========== SAVE RESULTS ==========
with open("hybrid_classification_report_HUP_hardvote_multiclass.txt", "w") as f:
    f.write("Hard voting ensemble results\n")
    f.write("\n\n" + report)
    f.write("\n\nConfusion Matrix:\n")
