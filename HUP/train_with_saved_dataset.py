...
for version, model_type in zip(version_suffixes, model_types):
    ...
    train_X, train_y = torch.load(f"{PREPROCESSED_DIR}/train_{version}_standard.pt")
    val_X, val_y = torch.load(f"{PREPROCESSED_DIR}/val_{version}_standard.pt")
    test_X, test_y = torch.load(f"{PREPROCESSED_DIR}/test_{version}_standard.pt")
    ...
    model_name = f"{model_type}_{version}_raw_standard"
...
merged_train_data = []
merged_train_labels = []
merged_val_data, merged_val_labels = torch.load(f"{PREPROCESSED_DIR}/merged_val_standard.pt")
...
# ========== PREDICT ON TEST SET ==========
X_test, y_test = torch.load(f"{PREPROCESSED_DIR}/merged_test_standard.pt")
