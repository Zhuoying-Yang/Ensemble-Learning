...
for version in version_suffixes:
    ...
    torch.save((train_X, train_y), f"{SAVE_DIR}/train_{version}.pt")
    torch.save((val_X, val_y), f"{SAVE_DIR}/val_{version}.pt")
    torch.save((test_X, test_y), f"{SAVE_DIR}/test_{version}.pt")
    print(f"[{version}] Saved train/val/test.")
...
torch.save((torch.cat(merged_val_data, dim=0), torch.cat(merged_val_labels, dim=0)),
           f"{SAVE_DIR}/merged_val.pt")
torch.save((torch.cat(merged_test_data, dim=0), torch.cat(merged_test_labels, dim=0)),
           f"{SAVE_DIR}/merged_test.pt")
print("Merged validation and test sets saved.")
