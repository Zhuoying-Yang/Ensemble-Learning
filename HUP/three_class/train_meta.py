...
class PerClassStackingEnsemble(nn.Module):
    def __init__(self, input_dim, hidden_dim=64, num_classes=3):
        ...
        self.heads = nn.ModuleList([
            nn.Sequential(
                ...
                nn.ReLU(),
                nn.Dropout(0.3),
                nn.Linear(hidden_dim, 1)  # binary logit for each class
            )
            for _ in range(num_classes)
        ])

    def forward(self, x):  # x shape: (N, M×3)
        outputs = [head(x) for head in self.heads]  # list of (N,1)
        return torch.cat(outputs, dim=1)  # shape: (N, 3)
...
X_train_stack = torch.tensor(np.concatenate(train_preds, axis=1), dtype=torch.float32).to(device)  # (N, M×3)
...
X_val_stack = torch.tensor(np.concatenate(merged_preds, axis=1), dtype=torch.float32).to(device)
y_val_true = merged_val_labels.long().to(device)


input_dim = X_train_stack.shape[1]  # should be num_models * 3
ensemble = PerClassStackingEnsemble(input_dim=input_dim).to(device)
optimizer = torch.optim.Adam(ensemble.parameters(), lr=0.001)
criterion = nn.BCEWithLogitsLoss()
y_train_onehot = torch.nn.functional.one_hot(y_train_true, num_classes=3).float()
for epoch in range(1000):
    ensemble.train()
    optimizer.zero_grad()
    logits = ensemble(X_train_stack)
    loss = criterion(logits, y_train_onehot)
    loss.backward()
    optimizer.step()

    if epoch % 50 == 0:
        val_logits = ensemble(X_val_stack)
        y_val_onehot = torch.nn.functional.one_hot(y_val_true, num_classes=3).float()
        val_loss = criterion(val_logits, y_val_onehot)
        val_pred = torch.sigmoid(val_logits).argmax(dim=1).cpu().numpy()
        val_acc = (val_pred == y_val_true.cpu().numpy()).mean()
        print(f"[Stacking] Epoch {epoch}: Train Loss = {loss.item():.4f} | Val Acc = {val_acc:.4f}")
...
X_test_stack = torch.tensor(np.concatenate(test_preds, axis=1), dtype=torch.float32).to(device)

with torch.no_grad():
    pred_logits = ensemble(X_test_stack)
    final_pred = torch.sigmoid(pred_logits).argmax(dim=1).cpu().numpy()
...
# ========== SAVE RESULTS ==========
with open("hybrid_classification_report_HUP_stacking_multiclass.txt", "w") as f:
    f.write("Stacking Ensemble Report\n\n")
    f.write(report)
    f.write("\n\nConfusion Matrix:\n")
